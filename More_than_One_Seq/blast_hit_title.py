...
def psiblast_out_process(psiblast_output_file):
    dufANDpfam = psiblast_output_file.split("/")
    dufID = dufANDpfam[2]
    pfamID = dufANDpfam[3].replace('_psiblast', '').split(".")[0]
    ...
    if int(blast_cnt.rstrip()) > 0:
        ...
        try:
            titles = open(f"../DUFs/{dufID}/{pfamID}_all_titles.txt", 'w')
        ...
        if cnt1 == 0:
            ...
            file_path = f"../DUFs/{dufID}/{pfamID}_all_titles.txt"
            ...
            for i in chars:
                cmd = f'sed -i "s/{i}//gI" ../DUFs/{dufID}/{pfamID}_all_titles.txt'
                os.system(cmd)
            ...
            for i in word_list:
                occ = os.popen(f'grep -i -c "{i[0]}" ../DUFs/{dufID}/{pfamID}_all_titles.txt').read()
            ...
            with open(f"../DUFs/{dufID}/{pfamID}_word_dict.txt", "w") as f:
                for kv in filter_word_list:
                    f.write(str(kv) + '\n')
            f.close()

            # writing deleted words to file:
            with open(f"../DUFs/{dufID}/{pfamID}_del_list.txt", "w") as f:
